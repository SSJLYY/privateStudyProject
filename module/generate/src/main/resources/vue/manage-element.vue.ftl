...
<#if !field.keyFlag>
      ...
      <el-form-item label-width="0px">
        ...
        <el-button icon="el-icon-plus" type="primary" @click="add${entity}()">添加</el-button>
...
<script>
  ...
  import {add${entity}, deleteBatch${entity}ById, delete${entity}ById, update${entity}} from "_a/admin/${entityLow}/${entityLow}";
  import { MessageError, MessageSuccess, ConfirmCustom } from '_l/message'

  export default {
    name: '${entity?replace("([a-z])([A-Z]+)","$1-$2","r")?lower_case}-manage',
    ...
    methods: {
      ...
      remove(row) {
        ConfirmCustom({type:'warning'}).then(()=>{
          delete${entity}ById(row.id).then(res => {
            if (res.data) {
              MessageSuccess('del')
            } else {
              MessageError('del')
