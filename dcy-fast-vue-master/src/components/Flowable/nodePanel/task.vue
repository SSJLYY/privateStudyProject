...
import {commonParse, userTaskParse} from '@/libs/flowable/common/parseElement'
...
export default {
  ...
  methods: {
    computedExecutionListenerLength() {
      let ele = this.element.businessObject.extensionElements
      if (ele !== undefined && ele !== null){
        let obj = ele.values.filter(item => item.$type === 'flowable:ExecutionListener');
        if (obj !== undefined && obj !== null) {
          this.executionListenerLength = obj.length;
        } else {
    ...
    computedTaskListenerLength() {
      let ele = this.element.businessObject.extensionElements
      if (ele !== undefined && ele !== null){
        let obj = ele.values.filter(item => item.$type === 'flowable:TaskListener')
        if (obj !== undefined && obj !== null) {
          this.taskListenerLength = obj.length;
        } else {
          this.taskListenerLength = 0;
        }
      }
    },
