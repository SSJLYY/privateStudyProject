...
export default {
  ...
  mounted() {
    let obj2 =this.element.businessObject.extensionElements.values
    ...
    this.formData.executionListener = obj2 != null ? obj2 :[];
  },
  methods: {
    configParam(index) {
      this.nowIndex = index
    ...
    updateElement() {
      if (this.formData.executionListener.length) {
        ...
        // 清除旧值
        let obj = extensionElements.values.filter(item => item.$type !== 'flowable:ExecutionListener')
        extensionElements.values =  obj != null ? obj : []
